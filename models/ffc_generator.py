...
class FFCGenerator(FFCModel):
    ...
    def __init__(self, nz: int, nc: int, ngf: int, g_factor: float = 0.5, debug: bool = False):
        ...
        self.ffc0 = FFC_BN_ACT(nz, ngf*8, 4, 0, g_factor, 1, 0, activation_layer=nn.ReLU, upsampling=True)
        self.ffc1 = FFC_BN_ACT(ngf*8, ngf*4, 4, g_factor, g_factor, 2, 1, activation_layer=nn.ReLU, upsampling=True)
        self.ffc2 = FFC_BN_ACT(ngf*4, ngf*2, 4, g_factor, g_factor, 2, 1, activation_layer=nn.ReLU,  upsampling=True)
        self.ffc3 = FFC_BN_ACT(ngf*2, ngf*2, 4, g_factor, g_factor, 2, 1, activation_layer=nn.ReLU,  upsampling=True)
